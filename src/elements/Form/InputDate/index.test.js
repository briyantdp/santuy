...
class TestDateInput extends Component {
  state = {
    value: {
      startDate: new Date(),
      endDate: new Date(),
...
const setup = () => {
  const { container } = render(<TestDateInput />);
...
test("Memiliki wrapper dengan className .form-date", () => {
  const { wrapper } = setup();

  expect(wrapper).toBeInTheDocument();
});

test("Memiliki input dengan className .form-control", () => {
  const { input } = setup();

  expect(input).toBeInTheDocument();
});

test("Memunculkan date picker ketika diklik", () => {
